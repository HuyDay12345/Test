...
const targetDate = new Date('9999-99-99');
...
if (!reqmethod || !target || !time || !threads || !ratelimit || !proxyfile) {
    console.clear();
    console.error(`
    Hulk v1 (Mod) Method With Bypass
    Developers to method: @linhcaptcha - recoding method 
    How to use & example:
...
if (currentDate > targetDate) {
    console.error('Error method has been outdate pm linhcaptcha');
    process.exit(1);
}

if (url.hostname.endsWith(blockedDomain)) {
    console.log(`Domain ${blockedDomain} blocked, if this mistake pm to tg @linhcaptcha`);
    process.exit(1);
}
...
if (isNaN(time) || time <= 0 || time > 120) {
    console.error('Error time can not high 120')
    process.exit(1);
}
