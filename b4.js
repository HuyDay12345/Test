...
const BROWSER_PROFILES = {
    "chrome": {
        ...
        getSpecificHeaders: (browser, version, platform, isMobile, fullVersion) => ({
            ...
            "upgrade-insecure-requests": Math.random() < 0.2 ? "1" : undefined, // Giảm từ 0.8
        })
    },
    "firefox": {
        platforms: ["Windows", "macOS", "Linux", "Android"],
...
const headerFunc = (parsedTarget, browser, version, platform, isMobile, fullVersion, geoCountryCode) => {
    ...
    } else if (secFetchSite === "cross-site" && Math.random() < 0.2) { // Giảm tỷ lệ xuống 20%
    ...
    const baseHeaders = {
        ...
        "dnt": Math.random() < 0.1 ? "1" : undefined, // Giảm từ 0.5
    };
